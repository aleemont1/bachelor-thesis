...
\chapter*{Abstract}
I razzi sperimentali rappresentano strumenti chiave per la ricerca aerospaziale,
meteorologica e per la validazione di nuove tecnologie in ambienti estremi.
In questo contesto, la disponibilità di sistemi di telemetria affidabili, a bassa
latenza e basso costo risulta fondamentale per il monitoraggio in tempo reale e
l’analisi post-missione.
Questa tesi presenta lo sviluppo di un sistema di telemetria embedded per il
tracciamento dei dati di volo di un razzo sonda sperimentale. Il sistema si basa
su un microcontrollore ESP32-S3 e utilizza la modulazione LoRa\textsuperscript{\textcopyright}
per la trasmissione dei dati verso una ground station, garantendo comunicazioni
stabili anche su lunghe distanze.
Per assicurare la robustezza e l’efficienza della trasmissione in assenza di
meccanismi di ritrasmissione, è stato progettato un protocollo di comunicazione
personalizzato, con segmentazione del payload, header identificativi, codice CRC
e gestione dei timeout per la ricostruzione dati.
I test in campo aperto hanno evidenziato una portata effettiva superiore ai 2 km. \sidenote{Inserire dati sul throughput effettivo}
Tali prestazioni risultano compatibili con i requisiti di missioni suborbitali a bassa quota.
...
In seguito si \'e proceduto a eseguire lo stesso test, ma variando l'angolo di
    ...
    \begin{table}[H]
        ...
        \resizebox{\textwidth}{!}{
            \begin{tabular}{|c|c|c|c|}
                \hline
                \textbf{Distanza} & \textbf{Frequenza di trasmissione} & \textbf{Potenza trasmissiva} & \textbf{Variabili di controllo}         \\ \hline
                10m               & \multirow{4}{*}{873.125 MHz}       & \multirow{4}{*}{+17 dBm}     & \multirow{4}{*}{Dimensione del payload} \\ \cline{1-1}
                50m               &                                    &                              &                                         \\ \cline{1-1}
                100m              &                                    &                              &                                         \\ \cline{1-1}
                500m              &                                    &                              &                                         \\ \hline
            \end{tabular}
