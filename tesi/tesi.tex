...
Visti i risultati dei test preliminari, discussi nel Capitolo \ref{chap:results},
i moduli \ac{LoRa} sono stati riconfigurati aumentando l'\ac{ADR} a 9.2~kbps, e
impostando il \emph{baud rate} a 115200bps sulla linea \ac{UART} dedicata.
Questo ha permesso di aumentare il \emph{throughput} effettivo, diminuendo la
...
La configurazione personalizzata ha permesso di migliorare i risultati del test,
con una riduzione dei tempi di ricostruzione pari all'8.7\%  per \emph{payload}
segmentati in 4 e 5 \emph{chunk}, mentre le prestazioni per i \emph{payload} da 1 a 3 \emph{chunk}
sono calate in maniera misurabile ma non significativa (l'aumento dei tempi di ricostruzione è inferiore 1\%),
come viene mostrato nelle Figure \ref{fig:segmentation-reconstruction-avg-time-comparison}-\ref{fig:segmentation-dist-comparison}
...
dimostrando quindi una buona stabilità nei tempi di ricostruzione dei \emph{payload}
segmentati.
