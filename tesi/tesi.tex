...
\acrodef{EKF}{\emph{Extended Kalman Filter}}
\acrodef{CRC}{Controllo di Ridondanza Ciclica}
\acrodef{LoS}{\emph{Line of Sight}}
\acrodef{ADR}{\emph{Air Data Rate}}
\restoregeometry
\newpage
% \begin{center}
%     (DA FARE ALLA FINE)\\
...
I razzi sperimentali svolgono un ruolo cruciale nella ricerca e nello sviluppo
...
affidabile dell'orientamento, della velocità verticale e dell'altitudine del razzo.
...
Il modulo impiega la ricetrasmittente \ac{LoRa} \emph{Semtech LLCC68} e
...
un'alta precisione di puntamento, difficile da mantenere senza
...
\begin{itemize}
    \item \textbf{Modulo \ac{LoRa} trasmettitore (Razzo)}: EByte E220-900T22D per
          ...
          omnidirezionale (vedi Sezione \ref{sec:antennas}).
    ...
    \item \textbf{Modulo \ac{LoRa} ricevitore (Ground Station)}: EByte E220-900T22D,
          ...
          ricevere i dati con antenna direzionale Yagi (vedi Sezione \ref{sec:antennas}).
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4cm}|p{6.5cm}|p{4cm}|}
        ...
        \hline
        Throughput                        & Valutare la velocità effettiva di trasmissione su diversi scenari di distanza e ambiente                                                                                                 & Pacchetti/s, bit/s, \ac{RSSI} \\
        \hline
...
Utilizzando una configurazione punto-punto, sono state effettuate misurazioni del \ac{RSSI}
...
I moduli sono stati configurati con i parametri di default descritti in \dsref{ds:lora}.
...
\subsubsection{Test del \emph{throughput}} \label{test:throughput-sec}
...
\paragraph{Dettagli operativi del setup sperimentale:} \label{test:throughput-setup}
Sulla base dei risultati dei test preliminari, discussi nel Capitolo~\ref{chap:results},
i moduli \ac{LoRa} sono stati riconfigurati, impostando un \ac{ADR} di 9.2~kbps
e configurando il \emph{baud rate} della linea \ac{UART} a 115200~bps.
Questa configurazione ha consentito di aumentare il \emph{throughput} effettivo del sistema,
a fronte di una riduzione della portata massima di trasmissione.
...
\begin{itemize}
    \item \textbf{Scenario con ostacoli:} la trasmittente era posizionata dietro
          un leggero rilievo e fitta vegetazione, in assenza di linea di vista diretta (non-\ac{LoS});
...
scelta per garantire una condizione di \ac{LoS} più favorevole:
...
Il test è stato effettuato sia con la configurazione di default che con la configurazione
descritta nella Sezione \ref{test:throughput-setup}.
...
\subsection{Configurazione personalizzata dei moduli radio}
Come descritto nella Sezione \ref{test:throughput-setup}, in seguito ai test
preliminari svolti, i parametri di configurazione dei moduli sono stati modificati
per aumentare l'\ac{ADR} e il \emph{baud rate} dell'interfaccia \ac{UART} del modulo.
Questo ha permesso di migliorare il \emph{throughput}, con i risultati descritti nella
Sezione \ref{test:segmentation}.
...
\ifdefined\HCode
    \begin{table}[H]
        ...
        \begin{tabular}{|c|c|c|c|c|c|c|c|c|c|c|}
            \hline
            \textbf{Distanza} & \textbf{\ac{RSSI} [\%] Medio} & \textbf{\ac{RSSI} [\%] $\sigma$} & \textbf{\ac{RSSI} [\%] Min} & \textbf{\ac{RSSI} [\%] Max} & \textbf{Throughput [Byte/s] Medio} & \textbf{Throughput [Byte/s] $\sigma$} & \textbf{Throughput [Byte/s] Min} & \textbf{Throughput [Byte/s] Max} & \textbf{Pkt/s Medio} & \textbf{Pkt/s $\sigma$} \\
            \hline
    ...
    \begin{table}[H]
        ...
        \resizebox{0.9\textwidth}{!}{
            \begin{tabular}{|c|cccc|cccc|cc|}
                ...
                \cline{2-11}
                                  & \textbf{Media}                          & \textbf{$\sigma$}                                 & \textbf{Min}                        & \textbf{Max}
                                  & \textbf{Media}                          & \textbf{$\sigma$}                                 & \textbf{Min}                        & \textbf{Max}
                                  & \textbf{Media}                          & \textbf{$\sigma$}                                                                                                                                       \\
                \hline
                10 m              & 86.25                                   & 0.54                                              & 84.31                               & 87.84        & 646.46 & 0.19  & 644.74 & 646.86 & 3.30 & 0.00 \\ \hline
                50 m              & 73.43                                   & 0.78                                              & 69.41                               & 75.29        & 646.44 & 0.16  & 644.74 & 646.86 & 3.30 & 0.00 \\ \hline
                100 m             & 70.18                                   & 0.86                                              & 69.02                               & 71.76        & 646.49 & 0.09  & 646.15 & 646.86 & 3.30 & 0.00 \\ \hline
...
mostra l’andamento del \ac{RSSI} medio in funzione della distanza, con una regressione
...
crescita del valore medio del \ac{RSSI} a 200~m, attribuibile alla posizione sopraelevata
...
\begin{figure}[H]
    ...
    \caption{Andamento del \ac{RSSI} in funzione della distanza (regressione logaritmica).}
...
\begin{figure}[H]
    ...
    \caption{Distribuzione del \ac{RSSI} per ciascuna distanza testata.}
...
dei \emph{payload} segmentati.

\paragraph*{Configurazione di default:}
Il tempo medio di ricostruzione cresce in maniera approssimativamente lineare
con il numero di \emph{chunk}, come mostrato in Figura~\ref{fig:segmentation-reconstruction-avg-time}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/tests/T2/T2-avg-time.png}
    \caption{Tempo medio di ricostruzione per numero di \emph{chunk}.}
...
\begin{figure}[H]
    ...
    \caption{Deviazione standard dei tempi di ricostruzione per numero di \emph{chunk}.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.47\textwidth}
        ...
        \caption{Distribuzione dei tempi di ricostruzione per 2 \emph{chunk}}
    ...
    \begin{minipage}{0.47\textwidth}
        ...
        \caption{Distribuzione dei tempi di ricostruzione per 3 \emph{chunk}}
    ...
    \begin{minipage}{0.47\textwidth}
        ...
        \caption{Distribuzione dei tempi di ricostruzione per 4 \emph{chunk}}
    ...
    \begin{minipage}{0.47\textwidth}
        ...
        \caption{Distribuzione dei tempi di ricostruzione per 5 \emph{chunk}}
        \label{fig:segmentation-test-5chunks}
    \end{minipage}
\end{figure}

\paragraph*{Configurazione personalizzata:}
La variabilità dei dati è rimasta contenuta, come si vede nelle Figure
\ref{fig:segmentation-reconstruction-sd-new}-\ref{fig:segmentation-test-5chunks-new},
confermando una buona stabilità nei tempi di ricostruzione dei \emph{payload}
segmentati.
\vspace{1cm}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/tests/T2/T2-avg-time-new.png}
    \caption{Tempo medio di ricostruzione per numero di \emph{chunk}.}
    \label{fig:segmentation-reconstruction-avg-time-new}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/tests/T2/T2-sd-new.png}
    \caption{Deviazione standard dei tempi di ricostruzione per numero di \emph{chunk}.}
    \label{fig:segmentation-reconstruction-sd-new}
\end{figure}
\begin{figure}[H]
    \centering
    \begin{minipage}{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/tests/T2/T2-dist-2chunks-new.png}
        \caption{Distribuzione dei tempi di ricostruzione per 2 \emph{chunk}}
        \label{fig:segmentation-test-2chunks-new}
    \end{minipage}
    \hfill
    \begin{minipage}{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/tests/T2/T2-dist-3chunks-new.png}
        \caption{Distribuzione dei tempi di ricostruzione per 3 \emph{chunk}}
        \label{fig:segmentation-test-3chunks-new}
    \end{minipage}
    ...
    \begin{minipage}{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/tests/T2/T2-dist-4chunks-new.png}
        \caption{Distribuzione dei tempi di ricostruzione per 4 \emph{chunk}}
        \label{fig:segmentation-test-4chunks-new}
    \end{minipage}
    \hfill
    \begin{minipage}{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/tests/T2/T2-dist-5chunks-new.png}
        \caption{Distribuzione dei tempi di ricostruzione per 5 \emph{chunk}}
        \label{fig:segmentation-test-5chunks-new}
    \end{minipage}
\end{figure}
\newpage
\paragraph*{Confronto tra le configurazioni:}
Dopo aver analizzato separatamente le due configurazioni, è stato eseguito un
confronto diretto.
La configurazione personalizzata ha mostrato una riduzione dei tempi di
ricostruzione fino all'8.7\% per \emph{payload} segmentati in 4 e 5 \emph{chunk},
con penalità trascurabili (inferiore all'1\%) sui payload più piccoli.
Le Figure~\ref{fig:segmentation-reconstruction-avg-time-comparison}-\ref{fig:segmentation-dist-comparison}
mostrano il confronto dettagliato.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/tests/T2/T2-avg-comparison.png}
    \caption{Confronto del tempo medio di ricostruzione del payload per numero di \emph{chunk}}
    \label{fig:segmentation-reconstruction-avg-time-comparison}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/tests/T2/T2-diff-comparison.png}
    \caption{Differenza percentuale tra i tempi di ricostruzione del payload, configurazione di default vs configurazione personalizzata}
    \label{fig:segmentation-diff-comparison}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/tests/T2/T2-dist-comparison.png}
    \caption{Confronto delle distribuzioni dei tempi di ricostruzione del payload, configurazione di default vs configurazione personalizzata}
    \label{fig:segmentation-dist-comparison}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img/tests/T5/T5.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img/tests/T6/T6.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img/tests/T5-6/T5-6.png}
...
% Infine, a \emph{Mari}, la mia metà. Non basterebbe un libro per scrivere tutto
% quello per cui dovrei ringraziarti.
% In questi anni mi hai spronato a essere la versione migliore di me, hai visto in
% me ciò che io non vedevo, mi hai fatto strada, prendendomi per mano, aspettandomi
% quando io avevo bisogno di riprendere fiato, trascinandomi di peso quando volevo
% fermarmi, e illuminando il cammino quando tutto era buio, con la pazienza che
% spesso non mi meritavo.
% Mi hai letto dentro, hai capito e accolto ogni mio \emph{meltdown}, mi hai
% supportato nei momenti di difficoltà, ma la cosa per cui più io ti ringrazio oggi,
% è avermi concesso di affrontare questo percorso insieme a te, la mia persona preferita.
% Grazie, con amore.
