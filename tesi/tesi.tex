...
\begin{itemize}
    \item \textbf{Modulo \ac{LoRa} trasmettitore (Razzo)}: EByte E220-900T22D per
          la trasmissione della telemetria alla \emph{ground station} con antenna omnidirezionale (Sez. \ref{sec:antennas}).
          Collegato tramite interfaccia \ac{UART} al computer di volo.
    \item \textbf{Modulo \ac{LoRa} ricevitore (Ground Station)}: EByte E220-900T22D, identico al ricevente,
          impiegato nella \emph{ground station} per ricevere i dati con antenna direzionale Yagi (Sez. \ref{sec:antennas}).
...
La progettazione del sistema ha tenuto conto di diverse sfide, tra cui l'ambiente
operativo difficile, vista la presenza di forti vibrazioni e accelerazioni estreme,
oltre alla presenza di possibili interferenze elettromagnetiche.
...
\sidenote{Inserire immagine schematica del pacchetto esteso}
...
modulare, applicando i principi di \emph{Clean Coding} e in un'ottica di espandibilità
futura.
...
\begin{itemize}
    \item \textbf{Modulo di raccolta dati}: gestisce la lettura dei sensori.
          \begin{figure}[H]
              \centering
          ...
          \end{figure}
    \item \textbf{Modulo di elaborazione dati}: si occupa dell'elaborazione dei dati
          raccolti dai sensori, applicando filtri e algoritmi di \emph{sensor fusion}.
    \item \textbf{Modulo di logging}: si occupa della serializzazione dei dati in formato JSON.
          \begin{figure}[H]
              \centering
          ...
          \end{figure}
          \newpage
    \item \textbf{Modulo di telemetria}: si occupa della serializzazione, segmentazione
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/uml/receiver.png}
